...
public class Swagger2 {
    ...
    //访问地址 http://localhost:8899/swagger-ui.html
   ...
   // public static final String SWAGGER_SCAN_ADMIN_PACKAGE = "com.zhicheng.iot.web.controller.api";
    ...
    public static final String SWAGGER_SCAN_APP_PACKAGE = "com.topband.bluetooth.api.controller";
...
*/
    ...
    public Docket createWxRestApi() {
        ParameterBuilder ticketPar = new ParameterBuilder();
        List<Parameter> pars = new ArrayList<Parameter>();
        ticketPar.name(SystemConstants.TOKEN_HEADER).description("user ticket")
                  .modelRef(new ModelRef("string")).parameterType("header")
                  .required(false).build(); //header中的ticket参数非必填，传空也可以
        pars.add(ticketPar.build());    //根据每个方法名也知道当前方法在设置什么参数
